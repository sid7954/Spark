...
if __name__ == "__main__":
    ...
    edges = lines.map(lambda node: (re.split(r"\t",node.strip())[0].lower(), re.split(r"\t",node.strip())[1].lower() ) ).distinct().groupByKey().partitionBy(int(sys.argv[3])).persist(pyspark.StorageLevel.MEMORY_AND_DISK) 
